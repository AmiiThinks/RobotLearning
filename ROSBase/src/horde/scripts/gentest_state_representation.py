    ...
    def get_phi(self, image, bump, ir, imu, odom, bias, weights=None,
        ...
        if (weights is not None):
            ...
            for pixel in range(StateConstants.NUM_RANDOM_POINTS):
                pixel_weights[pixel] = weights[
                                       StateConstants.PIXEL_FEATURE_LENGTH *
                                       pixel +
                                       StateConstants.IMAGE_START_INDEX:
                                       StateConstants.PIXEL_FEATURE_LENGTH * (
                                           pixel + 1) +
                                       StateConstants.IMAGE_START_INDEX].sum()

            sorted_pixels = np.absolute(pixel_weights).argsort()[
                            :GenTestStateConstants.NUM_POINTS_REPLACE]
            # retrieve the indices of the lowest pixels
            lowest_pixels = [pixel for pixel in sorted_pixels if
                             self.pixel_time_alive[
                                 pixel] >=
                             GenTestStateConstants.MIN_ALIVE_TIME]

            random_points = np.random.randint(low=0,
                                              high=StateConstants.IMAGE_LI *
            ...
            for pixel in lowest_pixels:
                ...
                # ensure uniqueness of new random points (shouldn't happen
                # often)
                ...
                weights[
                StateConstants.PIXEL_FEATURE_LENGTH * pixel +
                StateConstants.IMAGE_START_INDEX:
                StateConstants.PIXEL_FEATURE_LENGTH * (
                    pixel + 1) + StateConstants.IMAGE_START_INDEX] = 0


                # get the new state
        feature_state = super(GenTestStateManager, self).get_phi(image, bump,
                                                                 ir, imu, odom,
                                                                 bias, None,
                                                                 **kwargs)
