...
class GTD:
	...
	def update(self, phi, phi_prime, cumulant, gamma, rho):
		...
		self.theta = self.theta + self.alpha * (self.delta * self.e - gamma * (1 - self.lmbda) * np.dot(self.e, self.w) * phi_prime)
		self.w = self.w + self.beta * (self.delta * self.e - np.dot(phi, self.w) * phi)
